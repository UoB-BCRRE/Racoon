...
echo "Generic Racoon master, including amaas and icv"
export localdir=/Users/Jin/Documents/Ontology/Workplace/racoon-master
export rule=$localdir/rule
...
./stardog data add -g http://purl.org/rail/is/ $db $rail/rs/tbox.ttl
./stardog data add -g http://purl.org/rail/rs/ $db $rail/is/tbox.ttl
./stardog data add -g http://purl.org/rail/tt/ $db $rail/tt/tbox.ttl
./stardog data add -g http://purl.org/rail/rw/ $db $rail/weather/rw.tll

echo "Adding rule"
for file in "$rule"/*; do
    ./stardog data add $db "$file"
done
